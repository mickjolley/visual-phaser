...
Phased_Kit_Maker_V8.py creates phased paternal and maternal files.
...
from PKMconfig_V8 import (
    DNA_FOLDER, SIB_NAMES, PARENT, RELATIONSHIP, MODE, NO_CALLS
    )
...
def load_DNA_file(ind):
    ...
    for filname in file_names:
        if ind + "_raw_dna" in filname:
                    ...
                    try:                    
                        ...
                        try:                    
                            ...
                            df= df.drop(df[df["chromosome"] == "MT"].index)
                             ...
                             print(f'\n{this_file} could not be processed')
                             sys.exit()
            ...
            df= df.astype({'chromosome': int, 'position': int})
            df = df[df['chromosome'] <24]
            
            df = df.drop_duplicates(subset = ['chromosome','position'])
