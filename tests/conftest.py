...
The engine ships as `Visual_Phaser.V1.2.py` — a filename with dots in it, so it
cannot be imported by name. It is loaded here via importlib instead. Loading is
safe: the module body only reads config and defines functions, and the
executable entry point sits behind `if __name__ == "__main__"`.
"""
...
ENGINE_PATH = REPO_ROOT / "Visual_Phaser.V1.2.py"
