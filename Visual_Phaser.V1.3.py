# -*- coding: utf-8 -*-
"""
Visual_Phaser.V1.2.py performs comparisons between siblings and cousins and
...
def apply_conditions_vectorized(al1x, al2x, al1y, al2y, no_call_val):
    ...
    - Limegreen: Fully Identical (Both alleles match on both chromosomes).
    - Yellow: Half Identical (At least one allele matches).
    - Crimson: No match (Different alleles on both chromosomes).
    """
...
def thread_chromosome(chrom, match_pairs, individuals, files_path, map_positions, map_cms, chr_len, siblings, config_params):
    ...
    # Genetic analysis begins (DNA data is already cached in main thread)
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=min(len(individuals), 8)) as executor:
        ...
        for future in as_completed(futures):
            ind, ind_df, error_text = future.result()
            if ind_df is None or ind_df.empty:
                load_failures.append((ind, error_text or "Unknown parsing error."))
            else:
                with cache_lock:
                    worker_dna_cache[ind] = ind_df

    if load_failures:
        print("\n[VP_INPUT_ERROR] One or more individuals could not be loaded into usable DNA data.", flush=True)
        print("[VP_INPUT_ERROR] Check FILES_PATH and input file format (columns/delimiter/content).", flush=True)
        for ind, reason in load_failures:
